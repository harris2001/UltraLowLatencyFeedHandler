...
struct GapRecord {
    ...
    [[nodiscard]] constexpr uint64_t gap_size() const noexcept { return received_seq - expected_seq; }

    // Return session as a string_view
    [[nodiscard]] inline std::string_view session_view() const noexcept { return std::string_view(session, 10); }
};

static_assert(std::is_trivially_copyable_v<GapRecord>, "GapRecord must be trivially copyable for lock-free transfer");
static_assert(sizeof(GapRecord) == 40, "GapRecord size unexpected");

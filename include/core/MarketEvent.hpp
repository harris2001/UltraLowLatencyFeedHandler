 ...
 */
static_assert(sizeof(MarketEvent) == 64, "MarketEvent must be exactly one cache line (64 bytes)");

/**
 * Verify standard layout for binary compatibility.
 */
static_assert(std::is_standard_layout_v<MarketEvent>, "MarketEvent must be standard layout for binary compatibility");
